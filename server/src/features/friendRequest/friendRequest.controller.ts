...
async function getIncomingRequests(req: Request, res: Response, next: NextFunction) {
  try {
...
async function getOutgoingRequests(req: Request, res: Response, next: NextFunction) {
  try {
    const { userId } = req.params
...
async function acceptRequest(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const result = await friendRequestService.acceptRequest(senderId, receiverId)
    if (!result) 
...
async function getRequestStatusBetweenUsers(req: Request, res: Response, next: NextFunction) {
  try {
    const { senderId, receiverId } = req.query

    const request = await friendRequestService.getRequestStatusBetweenUsers(senderId as string, receiverId as string)
    if (!request)
