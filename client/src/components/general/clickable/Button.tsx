...
const Button: React.FC<ClickableProps> = ({ text, iconBegin, iconEnd, notSubmit, onClick, disabled, color, size, rounded, outlined, uppercased, innerWidth, innerHeight, className }) => {
  ...
  return (
    <button className={`${colorClass} ${size && "text-"+size} ${uppercased && "uppercase"} ${disabled && "pointer-events-none opacity-40"} ${innerWidth && "px-"+innerWidth} ${innerHeight && "py-"+innerHeight} ${rounded && "rounded-"+rounded} w-full flex flex-row justify-center items-center gap-x-1 tracking-wider font-semibold ${className}`}
      onClick={onClick}
